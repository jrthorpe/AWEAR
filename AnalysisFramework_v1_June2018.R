...
metrics.results.dates <- traj.summary %>% 
  summarise(AR.max = max(action.range),                    # maximum action range
            ...
            Tt.move = sum(durations[is.stay==0]),          # total time spent moving between stays
            Tm.move = mean(durations[is.stay==0]),         # average duration of a move
            Tt.out = sum(durations[!is.home]))             # total time spent out of the home

N.moves <- traj.summary %>% tally(loc.id==0) # number of moves
N.stay.out <- traj.summary %>% filter(!is.home) %>% tally(loc.id>0) # number of stays out of home
N.places <- traj.summary %>% filter(!is.home, loc.id>0) %>% distinct(loc.id) %>% tally() # number of unique places visited (not home)
...
metrics.results %<>% mutate(mcp.area = mcp.areas$mcp.area,
                            ...
                            N.places = N.places$n)
remove(N.moves, N.stay.out, N.places)
